...
pipeline {
    ...
    environment {
        ...
        IMAGE_NAME = 'homework'  // Name of the Docker image to be built
        ...
        NGINX = 'homework'  // Nginx container name, used for setting nginx configuration
    }
    ...
    stages {
        ...
        stage('2.Compile') {  // Second stage: Compile the application
            ...
            steps {
                ...
                sh 'cd ${WS} && npm install --registry=https://registry.npmmirror.com --no-fund --legacy-peer-deps && npm run build:${PROFILE}'  // Navigate to workspace, install dependencies, and build the project
            }
        }
        ...
        stage('4.Deploy') {  // Fourth stage: Deploy the application
            steps {
                ...
                //  Expo port : own port
                sh """
                    docker run -d --net ${NETWORK} -p 8800:80 \\
                      --name ${IMAGE_NAME} \\
                      --restart always \\
